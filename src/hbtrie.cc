                              ...
                              bool remove_key, int recDepth)
{
    ...
    if (rawkeylen == cur_chunk_no * chunksize) {
        ...
        if (given_valuebuf) {
            ...
            if (hv_meta.isVlenData()) {
                hv_meta = HBTrieValue(hbmeta.value, hbmeta.value_len);
            }
            hv_meta.toBinaryWoFlags(given_valuebuf);
            if (value_len_out) {
                *value_len_out = hv_meta.sizeWoFlags();
            }
        }
    ...
    if (kv_from_btree.keylen == suffix_len &&
        ...
        if (given_valuebuf) {
            if (hv_from_btree.isVlenData()) {
                hv_from_btree = HBTrieValue(kv_from_btree.value, kv_from_btree.valuelen);
            }
            hv_from_btree.toBinaryWoFlags(given_valuebuf);
            if (value_len_out) {
                *value_len_out = hv_from_btree.sizeWoFlags();
            }
        }
                                ...
                                int recDepth)
{
    ...
    if (flag & HBTRIE_PARTIAL_UPDATE) {
        ...
        if (oldvalue_out) {
            ...
            hv_from_btree.toBinaryWoFlags(oldvalue_out);
        }
        if (oldvalue_len_out) {
            *oldvalue_len_out = hv_from_btree.sizeWoFlags();
        }
